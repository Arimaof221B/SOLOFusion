...
class NuScenesDataset(Custom3DDataset):
    ...
    def __init__(self,
        ...
        self.img_ids = np.array(list(range(6019)))
        coco = COCO('data/nuimages/annotations/nuimages_v1.0-mini.json')
        self.cat_ids = coco.get_cat_ids(cat_names=self.CLASSES)
...
@DATASETS.register_module()
class NuScenesDataset2D(NuScenesDataset):
    r"""NuScenes Dataset.
        ...
        for idx in range(len(self)):
            ...
            for label in range(len(det)):
                ...
                for i in range(bboxes.shape[0]):
                    data = dict()
                    # data['image_id'] = img_id
                    data['token'] = self.data_infos[idx]['token']
                    data['bbox'] = self.xyxy2xywh(bboxes[i])
                    data['score'] = float(bboxes[i][4])
                    data['category_id'] = self.cat_ids[label]
                    data['category'] = self.CLASSES[data['category_id']]
