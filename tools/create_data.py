...
from tools.data_converter import nuscenes_solofusion as nuscenes_solofusion
...
def nuscenes_solofusion_data_prep(root_path,
                                info_prefix,
                                version,
                                dataset_name,
                                out_dir,
    ...
    Args:
        ...
        max_sweeps (int): Number of input consecutive frames. Default: 10
    """
    nuscenes_solofusion.create_nuscenes_infos(
        root_path, info_prefix, version=version, max_sweeps=max_sweeps)
    ...
    info_train_path = osp.join(root_path, f'{info_prefix}_infos_train.pkl')
    info_val_path = osp.join(root_path, f'{info_prefix}_infos_val.pkl')
    nuscenes_solofusion.export_2d_annotation(
        root_path, info_train_path, version=version)
    nuscenes_solofusion.export_2d_annotation(
        root_path, info_val_path, version=version)
    create_groundtruth_database(dataset_name, root_path, info_prefix,
...
if __name__ == '__main__':
    if args.dataset == 'kitti':
        ...
        nuscenes_data_prep(
            ...
            out_dir=args.out_dir,
            max_sweeps=args.max_sweeps)
    elif args.dataset == 'nuscenes_solofusion' and args.version != 'v1.0-mini':
        train_version = f'{args.version}-trainval'
        nuscenes_solofusion_data_prep(
            root_path=args.root_path,
            info_prefix=args.extra_tag,
            version=train_version,
            dataset_name='NuScenesDataset',
