...
def _fill_trainval_infos(nusc,
    ...
    with open('results_nusc.json', 'r') as f:
        pred_solofusion = json.load(f)['results']
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        if sample['token'] not in pred_solofusion.keys():
            continue
        boxes_global = [Box(box['translation'], box['size'], Quaternion(box['rotation']),
                            name=box['detection_name'], velocity=(*box['velocity'], 0)) for box in pred_solofusion[sample['token']]]        
        boxes_lidar = []
